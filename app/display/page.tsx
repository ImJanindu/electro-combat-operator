  ...
  return (
    ...
    >
      ...
      <div className="fixed inset-0 pointer-events-none">
        <div
          className={`absolute top-0 left-1/4 w-[600px] h-[600px] rounded-full blur-[180px] transition-all duration-1000 ${isRunning ? 'bg-neon-green/8' :
        ...
        />
        <div
          className={`absolute bottom-0 right-1/4 w-[600px] h-[600px] rounded-full blur-[180px] transition-all duration-1000 ${isRunning ? 'bg-neon-blue/5' :
      ...
      {/* ---- IDLE state ---- */}
      {isIdle && (
        <div className="relative z-20 text-center animate-slide-up flex flex-col items-center justify-center gap-6 md:gap-8 mt-24 md:mt-32">
          <p className="font-mono text-xs md:text-lg font-bold tracking-[0.15em] md:tracking-[0.25em] text-muted uppercase text-center max-w-4xl">
            MTSS of University of Sri Jayewardnepura Presents
          </p>
          <h1 className="font-mono text-5xl md:text-7xl font-black neon-text-cyan animate-neon-flicker tracking-tight text-center">
            ELECTRO COMBAT 2.0
          </h1>
          <p className="font-mono text-sm md:text-xl font-bold tracking-[0.25em] text-neon-red uppercase text-center mt-4 md:mt-8 animate-pulse-glow">
      ...
      {isMatchPhase && (
        <div className="relative z-20 text-center w-full max-w-5xl px-6">
          ...
          {/* Main Timer */}
          <div
            className={`timer-display text-[7rem] md:text-[10rem] lg:text-[12rem] font-black leading-none tracking-wider ${isRunning ? 'neon-text-green' :
          ...
          {/* Phase label */}
          <p
            className={`font-mono text-sm md:text-lg font-bold tracking-[0.3em] uppercase mt-4 ${isRunning ? 'text-neon-green' :
            ...
            <div className="mt-8 md:mt-12 animate-slide-up flex flex-col md:flex-row gap-4 justify-center">
              ...
              {(state.recoveryTimeA !== null || (isKnockout && state.recoveryTimeA === 0)) && (
                <div className={`flex flex-col items-center justify-center rounded-lg px-6 py-4 bg-surface/80 w-[280px] md:w-[340px] max-w-full ${state.recoveryTimeA === 0 ? 'neon-border-red' : 'neon-border-yellow'}`}>
                  ...
                  <div
                    className={`timer-display text-5xl md:text-7xl font-black ${state.recoveryTimeA === 0 ? 'neon-text-red animate-knockout-pulse' : 'neon-text-yellow animate-pulse-glow'
                      }`}
                  >
              ...
              {(state.recoveryTimeB !== null || (isKnockout && state.recoveryTimeB === 0)) && (
                <div className={`flex flex-col items-center justify-center rounded-lg px-6 py-4 bg-surface/80 w-[280px] md:w-[340px] max-w-full ${state.recoveryTimeB === 0 ? 'neon-border-red' : 'neon-border-yellow'}`}>
                  ...
                  <div
                    className={`timer-display text-5xl md:text-7xl font-black ${state.recoveryTimeB === 0 ? 'neon-text-red animate-knockout-pulse' : 'neon-text-yellow animate-pulse-glow'
                      }`}
                  >
